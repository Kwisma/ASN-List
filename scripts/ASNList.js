...
`;
    ...
    const files = [
        `./data/${name}/ASN.${name}.list`,
        `./data/${name}/ASN.${name}.yaml`,
        `./data/${name}/CIDR.${name}.list`,
        `./data/${name}/CIDR.${name}.yaml`,
    ];
...
function asnInfo(name, asnNumber) {
    ...
    fs.appendFileSync(`./data/${name}/ASN.${name}.yaml`, `payload:\n`, { encoding: 'utf8' });
    fs.appendFileSync(`./data/${name}/CIDR.${name}.yaml`, `payload:\n`, { encoding: 'utf8' });
}
...
`)
        ...
        for (let asn of asns) {
            ...
            if (asnName === 'ASN') {
                ...
                const output = `  - IP-ASN,${asnNumber},no-resolve\n`;
                fs.appendFileSync(`./data/${name}/ASN.${name}.yaml`, output, { encoding: 'utf8' });
